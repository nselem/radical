...
Radical.pm - contains methods for the RADICAL pipeline
...
sub load_aln{
    ...
    # get the taxa
    my $alnin = Bio::AlignIO->new(-file => "$alignfile", -format => "nexus");
...
sub parse_tree_bio_phylo{
    ...
    my $lineages = {};
    for my $node (@{$ladtree->get_internals}){
...
sub compare_all_nodes {
    ...
    my $TEnodes = keys %$TEtree;
    my $CFInodes = keys %$CFItree;
    my $CMNnodes = 0;
    ...
    my $MAXnodes = $TEnodes;
    ($MAXnodes = $CFInodes) if ($CFInodes > $MAXnodes);
    
    my $treesame;
    if ($CMNnodes == $MAXnodes){
	$treesame = 1;
    }
    else {
	$treesame = 0;
...
sub get_abortFrac{
    my $self = shift;
    return $self->{'configuration'}->{'ABORT_FRAC'};
}

sub get_abortInt{
    my $self = shift;
    return $self->{'configuration'}->{'ABORT_INT'};
}
